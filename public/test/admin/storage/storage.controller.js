...
describe('StorageController', function() {
  ...
  beforeEach(function() {
    getAllCachesExpectation = CacheServiceMock.expects('getAllCaches').resolves(mockCaches);
  ...
  it('should load the caches', function(done) {
    scope.$watch('caches', function(caches) {
      if (caches) {
        caches.length.should.be.equal(mockCaches.length);
        caches[0].totalSize.should.be.equal(2060392);
        CacheServiceMock.verify();
        getAllCachesExpectation.calledOnce.should.be.equal(true);
        done();
      }
    });
    scope.$apply();
  ...
  it('should load the maps', function(done) {
    scope.$watch('sources', function(maps) {
      if (maps) {
        ...
        maps[0].totalSize.should.be.equal(108);
        done();
      }
    });
    scope.$apply();
